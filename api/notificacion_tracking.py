...
def notificationTracking(batchID, contadorArchivos, procesador):
    ...
    apiTracking = event.obtenerVariableConnekta("Api_FacturaDigitalClaro_NotificacionTracking")
    ...
    # mensaje = event.notificacionResolucion()

    # Url
    notificationURL = f"{apiBase}{apiTracking}"
    ...
    for intento in range(intentos):
        try:
            # Notificacion 
            responseNotification = requests.post(notificationURL, json=jsonData)
            ...
            if intento < intentos - 1:
                ...
                logging.error("No se pudo establecer la conexion al servicio Tracking después de 3 intentos")
                print("No se pudo establecer la conexion al servicio Tracking después de 3 intentos")
                api.envioErrorConnekta(batchId='', tipoConexion="Servicio Lote Tracking", error=err, intentos=intentos)